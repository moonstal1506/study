...
import com.study.study.modules.account.form.Notifications;
import com.study.study.modules.account.form.Profile;
...
public class AccountService implements UserDetailsService {
    ...
    public void sendSignUpConfirmEmail(Account newAccount) {
        ...
        context.setVariable("message","합격스터디 서비스를 사용하려면 링크를 클릭하세요.");
        context.setVariable("host",appProperties.getHost());
        String message = templateEngine.process("mail/simple-link", context);

        EmailMessage emailMessage = EmailMessage.builder()
                .to(newAccount.getEmail())
                .subject("합격스터디, 회원 가입 인증")
                .message(message)
                .build();
    ...
    public void updateProfile(Account account, Profile profile) {
        modelMapper.map(profile, account);//profile에 있는 정보를 account에 맵핑
        accountRepository.save(account);//머지시킴
    }
